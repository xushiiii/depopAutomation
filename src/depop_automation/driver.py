...
def _new_edge_driver():
    ...
    if ATTACH_TO_EXISTING_EDGE:
        ...
        edge_options.add_argument(f"user-data-dir={paths.edge_user_data_dir}")
        edge_options.add_argument(f"profile-directory={paths.edge_profile_dir}")
    ...
    try:
        service = Service(paths.edge_driver_path, log_output=os.devnull)
    except TypeError:
        service = Service(paths.edge_driver_path, log_path=os.devnull)
